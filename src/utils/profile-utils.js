import { signProfileToken, wrapProfileToken } from 'aladinjs';
...
export function getProfileFromTokens(
  tokenRecords,
  publicKeychain,
  silentVerify = true
) {
  ...
  tokenRecords.map(tokenRecord => {
    ...
    if (decodedToken !== null) {
      profile = { ...profile, ...decodedToken.payload.claim };
    }
...
export function getDefaultProfileUrl(
  gaiaUrlBase: string,
  ownerAddress: string
) {
 ...
 */
export function fetchProfileLocations(
  gaiaUrlBase: string,
  ownerAddress: string,
  firstAddress: string,
  ownerIndex: number
      ...
      .then(response => {
        if (response.ok) {
          return response
            .json()
            .then(tokenRecords =>
              getProfileFromTokens(tokenRecords, ownerAddress, false)
            )
            .then(profile => 
              // logger.debug(`Found valid profile at ${location}`)
               ({ profile, profileUrl: location })
            )
            .catch(() => 
              // logger.debug(`Failed to verify profile at ${location}... trying others`)
               recursiveTryFetch(locations.slice(1))
            );
        }
