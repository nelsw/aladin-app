    ...
    if (tokenFileUrl) {
      fetch(tokenFileUrl)
        .catch(error => 
          ...
          //   error
          // );
           window.proxyFetch(tokenFileUrl)
          // return new Proxy(tokenFileUrl);
        )
        .then(response => response.text())
        .then(responseText => JSON.parse(responseText))
        .then(responseJson => {
          const tokenRecords = responseJson;
          const profile = getProfileFromTokens(
            tokenRecords,
            publicKeyOrAddress
          );
        ...
        .catch(error => {
          // logger.error(
          //   `resolveZoneFileToProfile: error fetching token file ${tokenFileUrl}`,
          //   error
          // );
          reject(error);
        });
